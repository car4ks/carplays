...
device.flashTimer = (device.flashTimer or 0) + dt
if device.flashTimer > 0.05 then  -- Reset every 0.05 seconds (twice as fast)
    device.flashTimer = 0
end
device.flashState = device.flashTimer < 0.025  -- On for 0.025 seconds, off for 0.025 seconds

if device.shiftAfterFireFuel <= 0 then
    -- Check for major throttle lift events and instant afterfire
    if device.instantAfterFireFuel > 800 and device.throttle < 0.3 and 
       device.instantEngineLoad < 0.2 and
       (device.lastInstantEngineLoad - device.instantEngineLoad) > 0.5 and
       device.outputAV1 > device.idleAV * 2 then
        electrics.values.afterFire = 1
        if device.instantAfterFireFuel > 1200 then
            electrics.values.afterFire2 = 1
        end
    end
    -- Check for sustained afterfire during deceleration with flashing
    if device.sustainedAfterFireFuel > 800 and 
       device.sustainedAfterFireTimer > 0 and
       device.throttle < 0.1 and
       device.outputAV1 > device.idleAV * 2 then
        electrics.values.afterFire = device.flashState and 1 or 0
        if device.sustainedAfterFireFuel > 1000 then
            electrics.values.afterFire2 = device.flashState and 1 or 0
        end
    end
else
    -- Shift backfires are direct
    if device.shiftAfterFireFuel > 800 then
        electrics.values.afterFire = 1
        electrics.values.afterFire2 = 1
    end
end

-- Reset lights when conditions aren't met
if device.instantAfterFireFuel <= 800 and 
   device.sustainedAfterFireFuel <= 800 and 
   device.shiftAfterFireFuel <= 0 then
    electrics.values.afterFire = 0
    electrics.values.afterFire2 = 0
end

device.lastInstantEngineLoad = device.instantEngineLoad
  end
...
local function setAntilagCoef(device, coef)
  device.antilagCoef = clamp(coef or 0, 0, 1)
  -- Store the antilag coefficient for use by the antilag system
end
